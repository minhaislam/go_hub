...
func main() {
	...
	// i := 0
	array_len := len(arrayTosort_tmp) / 2
	...
	for array_len >= 1 {
		...
		for i < len(arrayTosort_tmp) {
			...
			cycle_and_replace(arrayTosort_tmp, array_len, i)
			i++
		}
...
func cycle_and_replace(arrayTosort_tmp []int, array_len_in int, index_in int) {
	...
	if index_in-array_len_in >= 0 && arrayTosort_tmp[index_in] < arrayTosort_tmp[index_in-array_len_in] {
